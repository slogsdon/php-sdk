...
class TransITConnector extends XmlGateway implements IPaymentGateway
{
    ...
    public function processAuthorization(AuthorizationBuilder $builder)
    {
        ...
        if ($paymentMethod->cardType === 'Amex' && !empty($paymentMethod->cvn)) {
        ...
        if (!empty($builder->commercialData->lineItems)) {
            foreach ($builder->commercialData->lineItems as $lineItem) {
                ...
                //     $productTaxDetailsNode->appendChild($xml->createElement('productTaxName'));
                //     $productTaxDetailsNode->appendChild($xml->createElement('productTaxAmount'));
                //     $productTaxDetailsNode->appendChild($xml->createElement('productTaxPercentage'));
        ...
        if ($commercialDataSubmitted) {
            ...
            if (!empty($builder->commercialData->customerVAT_Number)) {
                $transaction->appendChild($xml->createElement('customerVATNumber', $builder->commercialData->customerVAT_Number));
            }
            ...
            if (!empty($builder->commercialData->VAT_InvoiceNumber)) {
                $transaction->appendChild($xml->createElement('vatInvoice', $builder->commercialData->VAT_InvoiceNumber));
            }
        ...
        $transaction->appendChild($xml->createElement('cardholderPresentDetail', $paymentMethod->cardPresent ? 'CARDHOLDER_PRESENT' : 'CARDHOLDER_NOT_PRESENT_ELECTRONIC_COMMERCE'));
