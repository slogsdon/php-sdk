...
final class DirectMarketingMOTO extends TestCase {
    ...
    public function test18StoreVisaCOF() {
        $card = $this->getVisa1();
        $card->cvn = null;
    ...
    public function test20MCCIT() {
        ...
        $mastercardMUT = $card->tokenize()
            ->execute();

        
        $card = $this->getMCUnclassifiedTIC();
        $card->token = $mastercardMUT->token;
        $card->number = null;
        
        $response = $card->charge(29.75)
            ->withCurrency('USD')
            ->withCardOnFile(true)
    ...
    public function test30RefundWithoutReference () {
        $response = $this->getVisa1()
            ->refund(7.00)
            ->withCurrency("USD")
            ->execute();
    ...
    public function test31RefundWithoutReference () {
        $response = $this->getMCUnclassifiedTIC()
            ->refund(5.00)
            ->withCurrency("USD")
            ->execute();
    ...
    // public function test32VoidARefund () {
